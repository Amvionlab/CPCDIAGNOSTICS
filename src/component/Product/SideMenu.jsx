...
function SideMenu({ setOpen, open, toggleDropdown, openDropdown }) {
  ...
  const handleNavigation = (category, section) => {
    ...
    if (section && section.trim() !== "") {
      ...
      navigate(`/${formattedCategory}/${formattedSection}`);
    } else {
  ...
  return (
    <div
      className={`sidebar tablet:w-[25vw] bg-white border rounded-md shadow-md p-2x tablet:py-2 tablet:px-4 tablet:sticky top-16 tablet:top-24 tablet:h-fit largeLaptop:top-32 laptop:h-[80vh] h-[80vh] largeLaptop:h-[50vh] overflow-y-auto z-10 ${open
          ...
          <div key={dropdown.category} className="rounded mb-2 list-group-item">
            <div
              ...
              onClick={() => {
                ...
                handleNavigation(dropdown.categoryURL, '')
              }}
            >
              ...
              <div className="border-t border-gray-300">
                {dropdown.subsection && dropdown.subsection.length > 0 ? (
                  dropdown.subsection.map((item, subIndex) => (
                    <div
                      onClick={() => handleNavigation(dropdown.categoryURL, item)}
                      key={subIndex}
                      className="no-underline"
                    >
                      <p
                        className="text-gray-800 px-4 py-2 capitalize tablet:text-base laptop:text-sm hover:bg-maincol hover:text-light-green cursor-pointer font-poppins
                      largeLaptop:text-lg"
                      >
                        {item}
                      </p>
