...
function Product() {
  ...
  const handleNavigation = (categoryURL, section) => {
    ...
    // Always navigate using absolute path (starts with '/')
    if (formattedSection) {
      navigate(`/${categoryURL}/${formattedSection}`, { replace: true });
    } else {
        ...
        <>
          <div className="font-poppins">
            ...
            <section className=" productSection largeLaptop:text-2xl py-2 text-base text-gray-700 laptop:mb-14">
              ...
              <div
                className="flex items-center gap-2 tablet:text-xl justify-start sm:hidden mb-4 mt-4"
                ...
                <div className="tablet:w-[75vw] bg-white p-1 tablet:p-4 text-center overflow-y-auto flex flex-col gap-2">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-2 laptop:grid-cols-3 gap-4 mb-4">
                    {final?.length > 0 ? (
                      final.map((item) => (
                        <Link
                          to={`/${selectedCategory}/${item.section}/${item.slug}`}
                          key={item.slug}
